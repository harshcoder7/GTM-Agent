...
# LinkedIn Posts Engagement Signal Flow as API Documentation

## Context
The LinkedIn Posts Engagement Signal Flow analyzes a person's recent LinkedIn activity to extract insights about their interests, writing style, and engagement patterns. This enables highly personalized outreach by understanding what topics and approaches resonate with the prospect.

## Input
**Format:** Plain text LinkedIn profile URL

**Required Parameter:**
- LinkedIn profile URL of the person to analyze
...
```json
{
  ...
  "engagement_signal_summary": {
    "core_themes": ["array of main topics/interests"],
    "writing_style": "description of communication style",
    "recent_focus": "current areas of emphasis",
    "engagement_trend": "posting frequency and audience response",
    "best_personalization_angle": "recommended approach for outreach",
    "example_opening_line": "sample personalized opening"
  }
}
```
...
## Process
1. **Post Scraping**
   - Takes LinkedIn URL from input
   - Scrapes recent posts according to predefined configuration settings
   - Extracts post content, engagement metrics, and metadata
   - Collects configurable number of recent posts for analysis

2. **Content Normalization**
   - Uses normalizer component to process raw posts data
   - Converts posts list into structured string format
   - Formats content for optimal LLM consumption as context
   - Maintains post chronology and engagement data

3. **Engagement Analysis**
   - Passes normalized posts string to LLM with specialized prompt
   - LLM analyzes content for themes, style, and engagement patterns
   - Identifies personalization opportunities and outreach angles
   - Generates comprehensive engagement signal summary

4. **Response Formatting**
   - Structures LLM analysis into standardized JSON output
   - Returns person name and detailed engagement insights

## Remarks
- **Configuration Driven:** Post scraping follows config settings for number of posts and date range
- **Normalization Critical:** Proper string formatting ensures consistent LLM analysis quality
- **Personalization Focus:** Output specifically designed to enable highly targeted outreach messaging


see image pis_1

and thi si sthe promtp which we gave to lml model  and then it goes to markdown json parser
...
- Strong angles to peronalize an outreach message

Output JSON format:
{
  "person_name": "<Name of the person from context>",
  "engagement_signal_summary": {
    "core_themes": [List of top 3–5 themes],
    "writing_style": "Brief description of tone and voice",
    "recent_focus": "What topics or events they are currently engaging with",
    "engagement_trend": "How active or consistent they are, or how people engage with their content",
    "best_personalization_angle": "What kind of message or reference would resonate best",
    "example_opening_line": "A first-line you might use in a cold email or message that shows you actually read their posts"
  }
}
