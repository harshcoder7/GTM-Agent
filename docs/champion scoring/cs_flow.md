# Champion Scoring Flow Documentation

## Context
The Champion Scoring Flow evaluates individual prospects within a company to identify high-value decision makers and influencers. It analyzes LinkedIn profiles to determine a person's likelihood to champion your product based on their role, experience, and alignment with company ICP results.

## Input
**Format:** JSON object

**Required Fields:**
- `linkedin_url` - LinkedIn profile URL of the person to score
- `icp_result` - JSON containing company ICP profiling results from previous flow

## Output
**Format:** JSON response

**Fields:**
- `champion_score` - Numerical/categorical score indicating champion potential
- `reasoning` - Detailed explanation for the scoring decision
- `product_fit` - How well the person aligns with your product's target persona

## Process
1. **Profile Scraping**
   - Takes LinkedIn URL from input JSON
   - Uses Apify scraper to extract comprehensive profile data
   - Retrieves professional information, experience, skills, and activity

2. **Profile Normalization**
   - Processes raw Apify response data
   - Filters and retains only relevant profile information
   - Restructures data into standardized format for LLM processing
   - Removes unnecessary fields and formats key data points

3. **Champion Analysis**
   - Combines normalized profile data with company ICP results
   - Passes combined data to LLM with champion scoring prompt
   - LLM evaluates decision-making authority, influence, and product alignment
   - Generates champion score, reasoning, and product fit assessment

4. **Response Generation**
   - Formats analysis results into structured JSON response
   - Returns scoring metrics and justification

## Remarks
- **Profile Enrichment:** Should modify to return normalized profile data in output for potential data enrichment use cases
- **Apify Dependency:** Flow relies on Apify scraping service availability and ratea limits
- **Data Quality:** Normalization step is crucial for consistent LLM input and scoring accuracy

this image has the overall flow @cs_1.webp

code for apify actor
...
this is teh flow in continuation  and we are pasisng the normalized profile and icp to an agent /llm with this prompt 

You are a “Champion Scoring Assistant.”

Your job is to take in two JSON objects:

1. `normalizedProfile`: a person’s LinkedIn profile distilled into structured fields.
2. `icpProfile`: the ICP profiling output, used only for context—do **not** factor its numeric `ICP Score` into your decision.
...
Produce **strict JSON** with exactly three keys:

- `champion_score`: an integer 0–10 reflecting their individual outreach-fit (not company fit).
- `reasoning`: 2–4 bullet points explaining why you scored them as you did, referencing profile attributes only.
- `product_fit`: echo the `Product Fit` field from `icpProfile` for downstream use.

Do **not** mention or weigh the `ICP Score` in your reasoning.

and then finally we have markdown json parser with this code
...
class MarkdownJSONParserComponent(Component):
    display_name = "Markdown JSON Parser"
    description = "Parse JSON from markdown code blocks (```json format)"
    documentation: str = "https://docs.langflow.org/components-custom-components"
    icon = "code"
    name = "MarkdownJSONParserComponent"
    
    inputs = [
        MessageTextInput(
            name="markdown_text",
            display_name="Markdown Text",
            info="Text containing JSON in markdown code blocks",
    ...
    outputs = [
        Output(display_name="Parsed JSON Data", name="json_output", method="parse_markdown_json"),
    ]
    
    def parse_markdown_json(self) -> Data:
        try:
            # Extract JSON from markdown code blocks
            # Look for ```json ... ``` or ``` ... ``` patterns
            json_pattern = r'```(?:json)?\s*\n?(.*?)\n?```'
            matches = re.findall(json_pattern, self.markdown_text, re.DOTALL)
            
            if not matches:
                # Try to find JSON without code blocks (fallback)
                json_text = self.markdown_text.strip()
            else:
            ...
            # Create Data object with parsed content
            result = Data(data=parsed_data)
            
            # Set status to indicate success
            if isinstance(parsed_data, dict):
                self.status = f"Successfully parsed JSON dict with {len(parsed_data)} keys"
            else:
                self.status = f"Successfully parsed JSON: {type(parsed_data).__name__}"
            
            return result
            
        except json.JSONDecodeError as e:
            # Handle JSON parsing errors
            error_data = Data(data={"error": f"JSON parsing failed: {str(e)}", "raw_text": json_text if 'json_text' in locals() else self.markdown_text})
            self.status = f"JSON parsing error: {str(e)}"
            return error_data
            
        except Exception as e:
            # Handle other unexpected errors
            error_data = Data(data={"error": f"Unexpected error: {str(e)}", "raw_input": self.markdown_text})
